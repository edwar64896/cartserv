...
sd788t_param_change sd788t_get_parameter_change_status(struct sp_port * _sp_port) {
	sd788t_param_change sdpc;
	_sd788t_get_parameter_change_status(_sp_port,&sdpc.version,&sdpc.takeflags,&sdpc.takehandle,&sdpc.parflags1,&sdpc.parflags2,&sdpc.parflags3);
	return sdpc;
}

uint8_t _sd788t_get_parameter_change_status(struct sp_port * _sp_port, uint8_t *version, uint8_t *takeflags, uint32_t *takehandle, uint8_t *parflags1, uint8_t *parflags2, uint8_t *parflags3 ) {

	CALLHEADER;

	rxbuf=sd788t_sendCommand(_sp_port,85,NULL,0);
	if (rxbuf == NULL) {
		return SD788T_RXBUF_NULL;
	}
...
uint8_t sd788t_ctrl_transport(struct sp_port * _sp_port, uint8_t uTransport) {

	CALLHEADER;

	uint8_t cmddata[]={uTransport};
	rxbuf=sd788t_sendCommand(_sp_port,17,cmddata,1);
	if (rxbuf == NULL) {
		return SD788T_RXBUF_NULL;
	}
...
uint8_t sd788t_get_transport(sd788t_sp_port * _sp_port) {
	uint8_t ui;
	_sd788t_get_transport(_sp_port,&ui);
	return ui;
}


unsigned char _sd788t_get_transport(sd788t_sp_port * _sp_port, unsigned char * pTransportStatus) {
	CALLHEADER;

	rxbuf=sd788t_sendCommand(_sp_port,18,NULL,0);
	if (rxbuf == NULL) {
		return SD788T_RXBUF_NULL;
	}
...
uint8_t sd788t_get_devicename(char *devicename) {
	...
	strcpy(devicename,(const char *)rxbuf->readBuf+1);
...
float sd788t_get_temperature(struct sp_port * _sp_port) {
	float uf;
	_sd788t_get_temperature(_sp_port, &uf);
	return uf;
}

unsigned char _sd788t_get_temperature(struct sp_port * _sp_port, float * pTemperature) {

	CALLHEADER;

	rxbuf=sd788t_sendCommand(_sp_port,15,NULL,0);
...
sd788t_tc sd788t_get_timecode(sd788t_sp_port * _sp_port) {
	sd788t_tc tc;
	_sd788t_get_timecode(_sp_port,&tc.gen,&tc.rx,&tc.fl);
	return tc;
}


uint8_t _sd788t_get_timecode(sd788t_sp_port * _sp_port, sd788t_smpte * regen_tc, sd788t_smpte * ext_tc, sd788t_smpte * file_tc) {

	CALLHEADER;

	rxbuf=sd788t_sendCommand(_sp_port,19,NULL,0);
...
struct sd788t_rxbuf * sd788t_create_rxbuf(int size) {
	...
	rxbuf=(struct sd788t_rxbuf *)calloc(1,sizeof(struct sd788t_rxbuf));
	rxbuf->size=size;
	//rxbuf->readBuf=(uint8_t *)calloc(size,sizeof(uint8_t));
 ...
 */
struct sp_port * sd788t_connect(int nsport) {
...
uint8_t * sd788t_createCommandBuffer(uint8_t command, uint8_t cmddata[],uint8_t cmddatasize,uint8_t *bufsize) {
	...
	char sum2=0;
	uint8_t j=0;
	uint8_t *sendBuf=(uint8_t *)calloc(cmddatasize+10,sizeof(uint8_t));
	...
	for (int i = 0; i < j+2; i++)
	{
		if (i == j)
		{
			checksum1 = 255 - ((sum1+sum2) % 255); sendBuf[j] = (unsigned char) checksum1;
		}
		if (i == j+1)
		{
			checksum2 = 255 - sum1; sendBuf[j+1] = (unsigned char) checksum2;
		}
...
struct sd788t_rxbuf * sd788t_checkAndSend(int exception, struct sp_port* sp_port,uint8_t * sendBuf, uint8_t bufLen)
{
	...
	ewrap (sp_blocking_write(sp_port,sendBuf,bufLen,5));
	...
	//ewrap2 (nBytes=sp_blocking_read(sp_port,readbuf,200,50));
	//ewrap2 (nBytes=sp_blocking_read(sp_port,readbuf,200,10));
	//ewrap (nBytes=sp_nonblocking_read(sp_port,readbuf,200));
	//ewrap (nBytes=sp_nonblocking_read(sp_port,readbuf,200));

	while (sp_input_waiting(sp_port)) {
		sp_blocking_read(sp_port,readbuf+bufPtr,1,10);
		bufPtr++;
	}
...
if (size==14 && bufPtr==16) {
	//printf("*********************************** problemoi: %#02x%#02x%#02x%#02x%#02x%#02x%#02x%#02x%#02x%#02x%#02x\n",*(uint8_t*)readbuf+0,*(uint8_t*)readbuf+1,*(uint8_t*)readbuf+2,*(uint8_t*)readbuf+3,*(uint8_t*)readbuf+4,*(uint8_t*)readbuf+5,*(uint8_t*)readbuf+6,*(uint8_t*)readbuf+7,*(uint8_t*)readbuf+8,*(uint8_t*)readbuf+9,*(uint8_t*)readbuf+10);
	printf("************************************************\n");
}
	printf(" %#0x %#0x %#02x %#0x %#02x %#0x %#0x %#0x %#0x %#0x\n",
			*((uint8_t*)(readbuf+0)),
			*((uint8_t*)(readbuf+1)),
			*((uint8_t*)(readbuf+2)),
			*((uint8_t*)(readbuf+3)),
...
if (0) { // 170918 - ignore checksum to see if it fixes kernel buffer issues on rpi3b+.
	if (!sd788t_checkIncoming((uint8_t *)readbuf,size+3)) {
